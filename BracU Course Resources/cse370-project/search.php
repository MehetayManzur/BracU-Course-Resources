...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['search_course'])) {
    ...
    $sql = "SELECT outline, description, code, name, D_name FROM courses WHERE code = ?";
    ...
    if ($stmt) {
        ...
        if ($result->num_rows > 0) {
            ...
            $error_message = "Course not available.";
        }
        $stmt->close();
    } else {
...
<head>
    ...
    <title>Cyber Course Details</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="container">
        <div class="search-box">
            ...
            <h1>Course Details Interface</h1>
            
            <form method="POST" class="search-form" id="searchForm">
                <div class="input-group">
                    <input type="text" name="course_code" placeholder="Enter Course Code" required>
                </div>
                <button type="submit" name="search_course" class="search-btn">Initialize Search</button>
            </form>
			<div class="button-group">
                <a href="available.php" class="available-btn">Available Courses</a>
            </div>
            ...
            <?php if ($search_result): ?>
                <div class="result-box">
                    ...
                    <p><strong>Code:</strong> <?php echo htmlspecialchars($search_result['code']); ?></p>
                    <p><strong>Name:</strong> <?php echo htmlspecialchars($search_result['name']); ?></p>
                    
                    <p><strong>Department:</strong> <?php echo htmlspecialchars($search_result['D_name']); ?></p>
                    <p><strong>Outline:</strong> <?php echo htmlspecialchars($search_result['outline']); ?></p>
                    <p><strong>Description:</strong> <?php echo htmlspecialchars($search_result['description']); ?></p>
                </div>
            <?php endif; ?>
        </div>
    </div>
